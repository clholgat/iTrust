...
public class DiagnosesDAO {
	 ...
	 */
	public long add(DiagnosisBean bean) throws DBException, FormValidationException {//Possible cause of bug 5
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			...
			try{
				...
				throw new FormValidationException("Must select a diagnosis before adding one");
			}
			ps.executeUpdate();
